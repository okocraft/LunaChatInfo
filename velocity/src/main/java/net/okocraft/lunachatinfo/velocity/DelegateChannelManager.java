...
public class DelegateChannelManager extends ChannelManager {
    ...
    public DelegateChannelManager(@NotNull ChannelIdentifier identifier, @NotNull ProxyServer server, @NotNull ChannelManager delegate) {
    ...
    private void sendStrings(String... strings) {
        try (
            ...
            this.server.getAllServers().forEach(target -> target.sendPluginMessage(this.identifier, bytes));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
